<!-- Generated by dash0-cli; do not edit by hand. Regenerate with `make skill-bundle` and edit the hand-curated source at internal/skill/content/SKILL.md in the dash0-cli repository instead. -->
---
name: dash0-cli
...
`--api-url`, `--otlp-url`, `--auth-token`, `--dataset`, `--profile`, `--agent-mode` (env: `DASH0_AGENT_MODE`), `--color` (`semantic` or `none`), `--experimental`/`-X`, `--max-retries`. Run `dash0 --agent-mode --help` for the full, current list.
