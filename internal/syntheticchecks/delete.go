...
func newDeleteCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Delete a synthetic check by its ID. Use --force to skip the confirmation prompt.` + internal.CONFIG_HINT,
...
func runDelete(ctx context.Context, id string, flags *asset.DeleteFlags) error {
	confirmed, err := confirmation.ConfirmDestructiveOperation(
		fmt.Sprintf("Are you sure you want to delete synthetic check %q? [y/N]: ", id),
		flags.Force,
	)
	if err != nil {
		return err
