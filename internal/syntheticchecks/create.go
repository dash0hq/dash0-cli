...
func newCreateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Create a new synthetic check from a YAML or JSON definition file. Use '-f -' to read from stdin.` + internal.CONFIG_HINT,
		Example: `  # Create from a YAML file
  dash0 synthetic-checks create -f check.yaml

  # Create from stdin
  cat check.yaml | dash0 synthetic-checks create -f -

  # Validate without creating
  dash0 synthetic-checks create -f check.yaml --dry-run`,
