...
# Step 2: List SLOs and find the created asset by name
...
if [ -z "$ID" ]; then
  echo "FAIL: Could not find created SLO '$ASSET_NAME' in list"
  exit 1
fi
echo "Created SLO ID: $ID"
...
# Step 5: Re-import exported YAML via apply (round-trip)
...
if echo "$LIST_JSON" | jq -e --arg id "$ID" '.[] | select(.metadata.labels["dash0.com/id"] == $id)' > /dev/null 2>&1; then
  echo "FAIL: SLO '$ID' still exists after deletion"
  exit 1
fi
