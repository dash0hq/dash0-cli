...
for attempt in $(seq 1 "$MAX_ATTEMPTS"); do
  TABLE_OUTPUT=$("$DASH0" logs query --precision disabled --from now-5m --filter "test.id is ${UNIQUE_ID}" 2>/dev/null) || true
...
CSV_OUTPUT=$("$DASH0" logs query --precision disabled --from now-5m --filter "test.id is ${UNIQUE_ID}" -o csv)
echo "$CSV_OUTPUT"
...
JSON_OUTPUT=$("$DASH0" logs query --precision disabled --from now-5m --filter "test.id is ${UNIQUE_ID}" -o json)
