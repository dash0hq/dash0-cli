...
if ! LIST_JSON=$("$DASH0" recording-rules list --all -o json); then
...
if ! "$DASH0" recording-rules update "$ID" -f "${TMPDIR}/exported.yaml"; then
  echo "FAIL: recording-rules update failed"
  exit 1
fi
